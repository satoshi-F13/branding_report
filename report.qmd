...
ggplot(regional_comparison, aes(x = year, y = mean_return, color = region, group = region)) +
  geom_line(linewidth = 1) +
  geom_point() +
  theme_minimal() +
  labs(
...
ggplot(performance_combined, aes(x = mean_return, y = pct_outperformed, 
                                color = region, size = sharpe_ratio)) +
  geom_point(alpha = 0.7) +
  geom_text(aes(label = country), hjust = -0.2, vjust = 0, size = 3) +
  scale_color_manual(values = c("Asia" = "#FF9933", "Europe" = "#003399")) +
  labs(
...
ggplot(yearly_volatility, aes(x = year, y = volatility, color = region, group = region)) +
  geom_line(linewidth = 1) +
  geom_point() +
  theme_minimal() +
  labs(
...
ggplot(all_comparisons, aes(x = country_return, fill = region)) +
...
crisis_performance <- all_comparisons %>%
  ...
  group_by(country, region, crisis) %>%
  summarize(
...
post_crisis_performance <- all_comparisons %>%
  ...
  group_by(country, region, recovery_period) %>%
  summarize(
    return = mean(country_return),
    .groups = 'drop'
  ) %>%
...
complete_data <- complete_grid %>%
  left_join(all_comparisons %>% select(year, country, country_return, region), 
            by = c("year", "country"))
...
risk_return_interactive <- ggplot(stability_metrics, 
  ...
  geom_point(aes(color = region, size = sharpe_ratio)) +
  scale_color_manual(values = c("Asia" = "#FF9933", "Europe" = "#003399")) +
  theme_minimal() +
  labs(
    title = "Risk vs. Return by Country",
